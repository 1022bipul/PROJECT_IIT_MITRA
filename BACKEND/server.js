...
const UserDetails = require("./src/models/userDetails.models");
...
const ImgVideoUrl = require("./src/models/imgVideoUrl");
...
//logout
 app.get("/account/logout", verifyToken, (req, res) => {
  return res
    .clearCookie("token")
    .status(200)
...
app.post('/accounts/details',async(req,res)=>{
  try {
...
// get url
app.get('/auth/profile/upload/image',verifyToken,async(req,res)=>{
  try {
    console.log('user auth')
    const email=res.user.userEmail;
    console.log(email)
